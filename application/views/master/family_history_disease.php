                    ...
                    <div class="row match-height">
                        <div class="col-xl-12 col-lg-12">
                            <div class="card">
                                ...
                                <h4 class="card-title">Family History Disease</h4>
                                </div>
                                <div class="card-content">
                                    <div class="card-body">
                                        ...
                                        <ul class="nav nav-tabs">
                                            <li class="nav-item">
                                                <a class="nav-link active" id="base-tab1" data-toggle="tab" aria-controls="tab1" href="#tab1" aria-expanded="true">Add Family History Disease</a>
                                            </li>
                                            <li class="nav-item">
                            ...
                            <form id="Family_history_disease" action="#" method="post"> 
...
$( document ).ready(function() {
   table= $('#tableid').DataTable( {
          'processing': true,
          'serverSide': true,
          'ajax':'<?=base_url()?>master/Family_history_disease/ajax_call',
          'columns': [
             { data: 'slno' },
             { data: 'name' },
             { data: 'description' },
...
function savedata()
{
    if( $("#name").val()=='') {
        ...
        $("#overlay").fadeIn(300);
         $.ajax({
            type: "POST",
            url: 'Family_history_disease/savedata',
            dataType: "json",
            data: $('#Family_history_disease').serialize(),
            success: function(data){
                $("#overlay").fadeOut(300);
               if(data.msg != ''){
               Swal.fire({title:"",text:""+data.msg+"",type:"success",confirmButtonClass:"btn btn-success",buttonsStyling:!1});
               table.draw();
                $("#Family_history_disease")[0].reset();
              } else if(data.error != ''){
...
function editdata(val)
{
    if(val>0)
    {
         ...
         $.ajax({
            type: "POST",
            url: 'Family_history_disease/editdata',
            dataType: "json",
            ...
            success: function(data){
               ...
               if(data.msg != ''){
                 ...
                 $('.modal-title').html('Edit Family History Disease');
              } else if(data.error != ''){
                Swal.fire({title:"Warning!",text:""+data.error+"",type:"warning",confirmButtonClass:"btn btn-primary",buttonsStyling:!1});
              } else if(data.error_message) 
              {
...
function updatedataval()
{
        ...
        $("#overlay").fadeIn(300);
         $.ajax({
            type: "POST",
            url: 'Family_history_disease/updatedata',
            dataType: "json",
          ...
          Swal.fire({title:"Are you sure?",
            ...
            buttonsStyling:!1}).then((function(t){
             ...
             $("#overlay").fadeIn(300);
                   $.ajax({
            type: "POST",
            url: 'Family_history_disease/deletedata',
            dataType: "json",
            data: {getid: val,csrf_test_name:$('#csrf_test_name').val()},
            success: function(data){
                $("#overlay").fadeOut(300);
