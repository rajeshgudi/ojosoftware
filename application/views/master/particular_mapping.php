                    ...
                    <div class="row match-height">
                        <div class="col-xl-12 col-lg-12">
                            <div class="card">
                                ...
                                <h4 class="card-title">Particular Mapping</h4>
                                </div>
                                <div class="card-content">
                                    <div class="card-body">
                                        ...
                                        <ul class="nav nav-tabs">
                                            <li class="nav-item">
                        <a class="nav-link active" id="base-tab1" data-toggle="tab" aria-controls="tab1" href="#tab1" aria-expanded="true">Add Eye Particular</a>
                                            </li>
                                            <li class="nav-item">
                        <a class="nav-link" id="base-tab2" data-toggle="tab" aria-controls="tab2" href="#tab2" aria-expanded="false">View/Edit/Delete</a>
                            ...
                            <form id="Particular_mapping" action="#" method="post"> 
                                <input type="hidden" name="csrf_test_name" id="csrf_test_name" value="<?php echo $this->security->get_csrf_hash(); ?>"> 
                                <div class="row">
                                <div class="col-md-4">
                                        <div class="form-group">
                                            <label for="lastname">Particular: <span class="text-danger">*</span></label>
                                            <select class="form-control select2" name="eye_complaints_id" id="eye_complaints_id">
                                              <option value="">Select Particular</option>
                                              <?php if($geteyecomp) foreach($geteyecomp as $datapart){ ?>
                                                <option value="<?php echo $datapart['eye_complaints_id'] ?>"><?php echo $datapart['name'] ?></option>
                                                <?php } ?>
                                           </select>
                                        </div>
                                    </div>
                                <div class="col-md-4" >
                                        <div class="form-group">
                                           ...
                                           <select class="form-control select2" name="left_eye" id="left_eye">
                                              <option value="">Select Left Eye</option>
                                              <?php if($left_eye) foreach($left_eye as $dataleye){ ?>
                                                <option value="<?php echo $dataleye['eye_details_id'] ?>"><?php echo $dataleye['name'] ?></option>
                                                <?php } ?>
                                           </select>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" name="right_eye" id="right_eye">
                                               <option value="">Select Right Eye</option>
                                               <?php if($right_eye) foreach($right_eye as $datareye){ ?>
                                                <option value="<?php echo $datareye['eye_details_id'] ?>"><?php echo $datareye['name'] ?></option>
                                                <?php } ?>
                                            </select>
                                        </div>
                                    </div>
...
$( document ).ready(function() {
   table= $('#tableid').DataTable( {
          'processing': true,
          'serverSide': true,
          'ajax':'<?=base_url()?>master/Particular_mapping/ajax_call',
          'columns': [
             { data: 'slno' },
             { data: 'name' },
             { data: 'description' },
...
function editdata(val)
{
    if(val>0)
    {
         ...
         $.ajax({
            type: "POST",
            url: 'Particular_mapping/editdata',
            dataType: "json",
            ...
            success: function(data){
               ...
               if(data.msg != ''){
                 ...
                 $('.modal-title').html('Edit Eye particular');
              } else if(data.error != ''){
                Swal.fire({title:"Warning!",text:""+data.error+"",type:"warning",confirmButtonClass:"btn btn-primary",buttonsStyling:!1});
              } else if(data.error_message) 
              {
...
function updatedataval()
{
        ...
        $("#overlay").fadeIn(300);
         $.ajax({
            type: "POST",
            url: 'Particular_mapping/updatedata',
            dataType: "json",
          ...
          Swal.fire({title:"Are you sure?",
            ...
            buttonsStyling:!1}).then((function(t){
             ...
             $("#overlay").fadeIn(300);
                   $.ajax({
            type: "POST",
            url: 'Particular_mapping/deletedata',
            dataType: "json",
            data: {getid: val,csrf_test_name:$('#csrf_test_name').val()},
            success: function(data){
                $("#overlay").fadeOut(300);
