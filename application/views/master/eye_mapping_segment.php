                    ...
                    <div class="row match-height">
                        <div class="col-xl-12 col-lg-12">
                            <div class="card">
                                ...
                                <h4 class="card-title">Eye Mapping Segment</h4>
                                </div>
                                <div class="card-content">
                                    <div class="card-body">
                                        ...
                                        <ul class="nav nav-tabs">
                                            <li class="nav-item">
                                                <a class="nav-link active" id="base-tab1" data-toggle="tab" aria-controls="tab1" href="#tab1" aria-expanded="true">Add Eye Segment</a>
                                            </li>
                                            <li class="nav-item">
                            ...
                            <form id="Eye_mapping_segment" action="#" method="post"> 
                                <input type="hidden" name="csrf_test_name" id="csrf_test_name" value="<?php echo $this->security->get_csrf_hash(); ?>"> 
                                <div class="row">
                                        <div class="col-md-4">
                                        <div class="form-group">
                                            <label for="lastname">Segment Type: <span class="text-danger">*</span></label>
                                           <select class="form-control select2" id="segment_type_id" name="segment_type_id" onchange="getsegmentlist(this.value)">
                                               <option value="">Select Segment Type</option>
                                               <option value="1">Anterior Segment</option>  
                                               <option value="2">Posterior Segment</option>  
                                               <option value="3">Non Segment</option>  
                                           </select>
                                        </div>
                                    </div>
                                       <div class="col-md-4">
                                        <div class="form-group">
                                            <label for="lastname">Segments Name list: <span class="text-danger">*</span></label>
                                           <select class="form-control select2" id="segment_id" name="segment_id">
                                               <option value="">Select Segments Name list</option>
...
$( document ).ready(function() {
   table= $('#tableid').DataTable( {
          'processing': true,
          'serverSide': true,
          'ajax':'<?=base_url()?>master/Eye_mapping_segment/ajax_call',
          'columns': [
             { data: 'slno' },
             { data: 'name' },
             { data: 'description' },
...
function getsegmentlist1(val,segmentid)
{
    $('#segment_idd').html('');
    if(val>0)
    {
        ...
        $("#overlay").fadeIn(300);
         $.ajax({
            type: "POST",
            url: 'Eye_mapping_segment/getsegmentlist1',
            dataType: "json",
           data: {segmenttypeid: val,segmentid: segmentid,csrf_test_name:$('#csrf_test_name').val()},
            success: function(data){
                $("#overlay").fadeOut(300);
               if(data.msg != ''){
               $('#segment_idd').html(data.datas);
              } else if(data.error != ''){
...
function getsegmentlist(val)
{
    if(val>0)
    {
        $("#overlay").fadeIn(300);
         $.ajax({
            type: "POST",
            url: 'Eye_mapping_segment/getsegmentlist',
            dataType: "json",
           data: {segmenttypeid: val,csrf_test_name:$('#csrf_test_name').val()},
            success: function(data){
                $("#overlay").fadeOut(300);
               if(data.msg != ''){
               $('#segment_id').html(data.datas);
              } else if(data.error != ''){
                Swal.fire({title:"Warning!",text:""+data.error+"",type:"warning",confirmButtonClass:"btn btn-primary",buttonsStyling:!1});
              } else if(data.error_message) 
              {
...
function editdata(val)
{
    ...
    {
         $("#overlay").fadeIn(300);
         $('#Eye_mapping_segment_instruction_idd').val('');
         $.ajax({
            type: "POST",
            url: 'Eye_mapping_segment/editdata',
            dataType: "json",
            ...
            success: function(data){
               ...
               if(data.msg != ''){
                 ...
                 $('.modal-title').html('Edit Eye Segment');
                getsegmentlist1(data.segment_type_id,data.eye_segment_id);
...
function updatedataval()
{
        ...
        $("#overlay").fadeIn(300);
         $.ajax({
            type: "POST",
            url: 'Eye_mapping_segment/updatedata',
            dataType: "json",
          ...
          Swal.fire({title:"Are you sure?",
            ...
            buttonsStyling:!1}).then((function(t){
             ...
             $("#overlay").fadeIn(300);
                   $.ajax({
            type: "POST",
            url: 'Eye_mapping_segment/deletedata',
            dataType: "json",
            data: {getid: val,csrf_test_name:$('#csrf_test_name').val()},
            success: function(data){
                $("#overlay").fadeOut(300);
