                    ...
                    <div class="row match-height">
                        <div class="col-xl-12 col-lg-12">
                            <div class="card">
                                ...
                                <h4 class="card-title">Past psychiatrist illness</h4>
                                </div>
                                <div class="card-content">
                                    <div class="card-body">
                                        ...
                                        <ul class="nav nav-tabs">
                                            <li class="nav-item">
                                                <a class="nav-link active" id="base-tab1" data-toggle="tab" aria-controls="tab1" href="#tab1" aria-expanded="true">Add Past psychiatrist illness</a>
                                            </li>
                                            <li class="nav-item">
                            ...
                            <form id="Past_psychiatrist_illness" action="#" method="post"> 
...
$( document ).ready(function() {
   table= $('#tableid').DataTable( {
          'processing': true,
          'serverSide': true,
          'ajax':'<?=base_url()?>master/Past_psychiatrist_illness/ajax_call',
          'columns': [
             { data: 'slno' },
             { data: 'name' },
             { data: 'description' },
...
function savedata()
{
    if( $("#name").val()=='') {
        ...
        $("#overlay").fadeIn(300);
         $.ajax({
            type: "POST",
            url: 'Past_psychiatrist_illness/savedata',
            dataType: "json",
            data: $('#Past_psychiatrist_illness').serialize(),
            success: function(data){
                $("#overlay").fadeOut(300);
               if(data.msg != ''){
               Swal.fire({title:"",text:""+data.msg+"",type:"success",confirmButtonClass:"btn btn-success",buttonsStyling:!1});
               table.draw();
                $("#Past_psychiatrist_illness")[0].reset();
              } else if(data.error != ''){
...
function editdata(val)
{
    if(val>0)
    {
         ...
         $.ajax({
            type: "POST",
            url: 'Past_psychiatrist_illness/editdata',
            dataType: "json",
            ...
            success: function(data){
               ...
               if(data.msg != ''){
                 ...
                 $('.modal-title').html('Edit Past_psychiatrist_illness');
              } else if(data.error != ''){
                Swal.fire({title:"Warning!",text:""+data.error+"",type:"warning",confirmButtonClass:"btn btn-primary",buttonsStyling:!1});
              } else if(data.error_message) 
              {
...
function updatedataval()
{
        ...
        $("#overlay").fadeIn(300);
         $.ajax({
            type: "POST",
            url: 'Past_psychiatrist_illness/updatedata',
            dataType: "json",
          ...
          Swal.fire({title:"Are you sure?",
            ...
            buttonsStyling:!1}).then((function(t){
             ...
             $("#overlay").fadeIn(300);
                   $.ajax({
            type: "POST",
            url: 'Past_psychiatrist_illness/deletedata',
            dataType: "json",
            data: {getid: val,csrf_test_name:$('#csrf_test_name').val()},
            success: function(data){
                $("#overlay").fadeOut(300);
