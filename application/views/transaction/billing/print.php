...
<html>
    <body>
     ...
     ?>
         ...
         $tot5=$grand_total;
         $tot1=$bill_amount-$advanced_amount;
         $hh='';
         if($dpaymentm)
         {
            $hh='<br/>';
      ...
      $amounpaid=$bill_amount;
      if($advanced_amount)
      {
        $amounpaid=$advanced_amount;
      }
      else {
      ...
      if($insuranceamount)
      {
        $amounpaid=$amounpaid-$insuranceamount;
      }
      else {
 ...
 <br>
            <table style="width:100%;font-size: 14px;">
                <tr>
                    <td style="width: 60%">
                        Total  Amount: <?=number_format($bill_amount,2) ?><br>
                        Insurance  Amount: <?=number_format($insuranceamount,2)?><br>
                        Advance Amount: <?=number_format($advanced_amount,2)?><br>
                        Amount Paid by Patient: <?=number_format($amounpaid,2)?><br>
                        <?=$mode?>: <?=number_format($amounpaid,2)?><br>
                        <?=$dpaymentm?> <?php echo $hh; ?>
                        Terms and Conditions:NB: <?=number_format($amounpaid,2)?>/-(<?=$net_amount_in_words?>) was directly collected from patient.
                    </td>
                 </tr>
            </table>
            <br/><br/><br/><br/>
