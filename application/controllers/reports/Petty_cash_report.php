...
class Petty_cash_report extends CI_Controller {
	...
	public function __construct() {
        ...
        $this->load->model('Petty_cash_report_model');
        $this->load->model('Common_model');
    }
	public function index()
	{
		$data['title']='Optical::Petty Cash Report';
		$data['activecls']='Petty_cash_report';
		$office_id=$this->session->office_id;
		$var_array=array($office_id);
		$content=$this->load->view('reports/petty_cash_report',$data,true);
    ...
    public function getsummary()
    {
      ...
      if($this->form_validation->run() == TRUE)
      {
        ...
        $petty_cash_name=trim(htmlentities($this->input->post('petty_cash_name')));
        $pettycash=$this->Petty_cash_report_model->pettycashdata($sum_fromdate,$sum_todate,$petty_cash_name,$this->session->userdata('office_id'));
		  	...
		  	$html='<table class="table table-striped table-bordered dataex-html5-selectors" id="example_sum">
                   ...
                   foreach($pettycash as $datafor)
                   {
                    ...
                    if($datafor['pay_type']==1)
                      ...
                      $sum+=$datafor['petty_amount'];
                     $html.='<tr>
                               <td>'.$sl.'</td>
                               <td>'.$datafor['petty_receipt_no'].'</td>
                               <td>'.$pp.'</td>
                               <td>'.$datafor['petty_cash_name'].'</td>
                               
                               <td>'.$datafor['petty_date'].'</td>
                               <td>'.$datafor['description'].'</td>
                               <td>'.number_format($datafor['petty_amount'],2).'</td>
