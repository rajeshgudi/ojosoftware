...
class Receipt extends CI_Controller {
	...
	public function __construct() {
     ...
     public function ajax_call(){
        ...
        $response=$this->Receipt_model->ajax_call($param);
...

	{
      ...
      $data['title']='EMR::Receipt';

      $data['activecls']='Receipt';
      ...
      $content=$this->load->view('receipt/billing/insert',$data,true);
...

  {
    ...
    error_reporting(0);
...

      {
          ...
          $getresult=$this->Receipt_model->savedata($data);
...

  {
...

      {
        ...
        $chk_duplication=$this->Receipt_model->editcheckbillingentry($var_array);
...

        {
          ...
          $getmasterdata=$this->Receipt_model->Getmastertable($var_array);

          $getdetailsdata=$this->Receipt_model->Getdetailstable($var_array_child);
...

  {
...

      {
        ...
        $getresult=$this->Receipt_model->updatedata($data,$edit_id);
...

  {
    ...
    $this->form_validation->set_rules('getid', 'Delete ID', 'trim|required|min_length[1]|max_length[100]|numeric');
...

      {
          ...
          $getresult=$this->Receipt_model->deletedata($delete_id);
...

  {
      ...
      $this->Receipt_model->print_bill($this->input->post('billing_id'),$this->session->userdata('office_id'));
