...
class Camp_billing extends CI_Controller {
	...
	public function __construct() {
     ...
     public function ajax_call(){
        ...
        $response=$this->Camp_billing_model->ajax_call($param);
...

	{
      ...
      $data['title']='EMR::Camp_Billing';

      $data['activecls']='Camp_Billing';
      ...
      $content=$this->load->view('transaction/camp_billing/insert',$data,true);
...

  {
    ...
    error_reporting(0);
...

      {
          ...
          $getresult=$this->Camp_billing_model->savedata($data);
...

  {
...

      {
        ...
        $chk_duplication=$this->Camp_billing_model->editcheckbillingentry($var_array);
...

        {
          ...
          $getmasterdata=$this->Camp_billing_model->Getmastertable($var_array);

          $getdetailsdata=$this->Camp_billing_model->Getdetailstable($var_array_child);
...

  {
...

      {
        ...
        $getresult=$this->Camp_billing_model->updatedata($data,$edit_id);
...

  {
...

      {
        ...
        $getresult=$this->Camp_billing_model->updatedataAdjust($data,$edit_id);
  ...
  public function Get_Package_List_particulars()
  {
      ...
      {  
           ...
           $var_array=array($sel);
           $getresult=$this->Camp_billing_model->Get_all_package($var_array);
           if($getresult)
           { 
               $html='';
...

  {
    ...
    $this->form_validation->set_rules('getid', 'Delete ID', 'trim|required|min_length[1]|max_length[100]|numeric');
...

      {
          ...
          $getresult=$this->Camp_billing_model->deletedata($delete_id);
...

  {
      ...
      $this->Camp_billing_model->print_bill($this->input->post('billing_id'),$this->session->userdata('office_id'));
  ...
  public function getsavedataAdjust()
  {
      ...
      if($this->form_validation->run() == TRUE)
      {
        ...
        $var_array=array($getid,$this->session->userdata('office_id'));
          ...
          $var_array_child=array($getid);
          $getmasterdata=$this->Camp_billing_model->Getmastertable($var_array);
          $getdetailsdata=$this->Camp_billing_model->Getdetailstable($getid);
          $sl=1;
          if($getdetailsdata)
          {
              foreach ($getdetailsdata as $data) {
