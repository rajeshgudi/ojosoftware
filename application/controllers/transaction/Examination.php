...
class Examination extends CI_Controller
{
...

    {
        ...
        if ($checkingpat) {
            ...
            if ($count_pa > 0) {
                echo json_encode(array('msg' => '', 'error' => 'This Patient Already Updated in optometry Login.Please go to back', 'error_message' => ''));
                exit;
            }
        }
  ...
  <h2 style="font-weight:bold;color:white">Uploaded Files</h2>
    ...
    public function getrefractioninputdata()
    {
        $var_array = array($this->session->userdata('office_id'));

        $getdata = $this->Common_model->getrefractionsinputdata($var_array);
        ...
        foreach ($getdata as $dataw) {
            $options = "";
            $final_data[$dataw['eye_type']][$dataw['ref_type']][] = $dataw['name']; //'<option value="'.$dataw['name'].'">' . $dataw['name'] . '</option>' ;
        ...
        echo (count($final_data) > 0) ? json_encode(array('msg' => $final_data, 'error' => '', 'error_message' => ''))  : json_encode(array('msg' => '', 'error' => 'No data Found', 'error_message' => ''));;
    }
    ...
    public function cross_ref_save()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($getresult_doc) {
                ...
                if ($cnt_same_doc > 0) {
                    echo json_encode(array('msg' => '', 'error' => 'Same doctor not allowed to Cross Referral', 'error_message' => ''));
                    exit;
                }
            }
