...
class Category extends CI_Controller {
  public function __construct() {
    ...
    public function ajax_call(){
        $param=$_REQUEST;
        $response=$this->Category_model->ajax_call($param);
        echo json_encode($response);
 }
	public function index()
	{
      $data['title']='EMR::Patient Category';
    ...
    private function fetch_data() 
    {
        ...
        if(!$this->input->post('investigation'))
        {
            $investigation=0;
        }
        else
    ...
    public function savedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$opd=trim(htmlentities($this->input->post('opd')));
	    	$ipd=trim(htmlentities($this->input->post('ipd')));
	    	$laser=trim(htmlentities($this->input->post('laser')));
	    	$injection=trim(htmlentities($this->input->post('injection')));
	    	...
	    	if($chk_duplication[0]['cnt']==0)
	    	{
	    		...
	    		$getresult=$this->Category_model->savedata($data);
	    		if($getresult)
	    		{
	...
	public function editdata()
	{
		...
		if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$getdata=$this->Category_model->geteditdata($var_array);
	    	if($getdata)
	    	{
	    		$html='';
	    		foreach($getdata as $data)
	    		{
	    			...
	    			if($data['injection']==1)
	    			{
	    				$injectionchk="checked";
	    			}
	    			if($data['investigation']==1)
	    			{
	    				$investigationchk='checked';
	    			}
							                ...
							                <div class="modal-body">
							                    <div class="row">
							                        <div class="col-lg-12 col-md-12" >
							                          <input type="hidden" name="id" id="id" value="'.$data['patient_category_id'].'">
							                           <input type="hidden" name="csrf_test_name" id="csrf_test_name" value="'.$this->security->get_csrf_hash().'"> 
							                                ...
							                                <div class="row col-md-12">
							                                   <div class="form-group">
							                                       OPD <input type="checkbox" '.$opdchk.' name="opd" id="opd"  >
							                                        IPD <input type="checkbox" '.$ipdchk.' name="ipd" id="edit_ipd"  >
							                                        laser <input type="checkbox" '.$laserchk.' name="laser" id="laser" >
							                                        Injection <input type="checkbox" '.$injectionchk.' name="injection" id="injection"  >
							                                        Investigation <input type="checkbox" '.$investigationchk.' name="investigation" id="investigation"  >
							                                    </div>
							                                </div>
							                                  <div class="row col-md-12">
							                                    <div class="col-md-12 form-group">
	...
	public function updatedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Category_model->editcheck($var_array);
	    	if($chk_duplication[0]['cnt']==0)
	    	{
	    		$data=$this->fetch_data();
	    		$getresult=$this->Category_model->updatedata($data,$edit_id);
	    		if($getresult)
	    		{
	...
	public function deletedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Category_model->deletecheck($var_array);
	    	if($chk_duplication[0]['cnt']==1)
	    	{
	    		$getresult=$this->Category_model->deletedata($delete_id);
	    		if($getresult)
	    		{
