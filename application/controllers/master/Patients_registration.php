...
class Patients_registration extends CI_Controller
{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($getresult) {
				...
				foreach ($getresult as $data) {
					...
					$age_array = calculate_age($data['dob']);
					$age_Yrs = $age_array["years"];
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($host_tvm == 'deh') {
				...
				if ($cardno) {
					...
					if ($cardavailable == 0) {
						echo json_encode(array('msg' => '', 'error' => 'This card not available in camp list tab .please check', 'error_message' => ''));
						exit;
					}
					if ($cardavailable > 0) {
						$cntcardno12 = 0;
						...
						if ($cntcardno12 > 12) {
							echo json_encode(array('msg' => '', 'error' => 'This card usage count is over', 'error_message' => ''));
							exit;
						}
					}
				}
	...
	public function checkduplicateuser()
	{
		...
		$getresult = $this->Patients_registration_model->checkduplicateuser(array("fname" => $fname, "mobileno" => $mobileno, "officeid" => $officeid));
		echo json_encode($getresult);
	}
}
