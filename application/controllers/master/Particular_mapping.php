...
class Particular_mapping extends CI_Controller {
  public function __construct() {
        ...
        $this->load->model('Particular_mapping_model');
    ...
    public function ajax_call(){
        $param=$_REQUEST;
        $response=$this->Particular_mapping_model->ajax_call($param);
        echo json_encode($response);
 }
	public function index()
	{
      $data['title']='EMR::Particular_mapping';
      $data['activecls']='Particular_mapping';
      ...
      $content=$this->load->view('master/particular_mapping',$data,true);
    ...
    private function fetch_data() 
    {
        ...
        return array(
            'left_eye'=>trim($this->input->post('left_eye')),
            'right_eye'=>trim($this->input->post('right_eye')),
            'eye_complaints_id'=>trim($this->input->post('eye_complaints_id')),
	...
	{
		$this->form_validation->set_rules('eye_complaints_id', 'eye_complaints_id', 'trim|required|min_length[1]|max_length[200]');
    	$this->form_validation->set_rules('description', 'Description', 'trim');
	    if($this->form_validation->run() == TRUE)
	    {
	    		$data=$this->fetch_data();
	    		$getresult=$this->Particular_mapping_model->savedata($data);
	    		if($getresult)
	    		{
	...
	public function editdata()
	{
		...
		if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$getdata=$this->Particular_mapping_model->geteditdata($var_array);
	    	if($getdata)
	    	{
	    		$html='';
...
$opvss2='';
					$var_array=array($this->session->userdata('office_id'));
      				$geteyecomp=$this->Examination_model->getallcomp($var_array);
					  if($geteyecomp)
					  {
						  foreach ($geteyecomp as $ddd) 
						  {
							  $sel12='';
							  if($ddd['eye_complaints_id']==$data['eye_complaints_id'])
							  {
								  $sel12='selected';
							  }
							  $opvss2.='<option value="'.$ddd['eye_complaints_id'].'" '.$sel12.'>'.$ddd['name'].'</option>';
						  }
					  }
							                ...
							                <div class="modal-body">
							                    <div class="row">
							                        <div class="col-lg-12 col-md-12" >
							                          <input type="hidden" name="id" id="id" value="'.$data['eye_particular_id'].'">
							                           <input type="hidden" name="csrf_test_name" id="csrf_test_name" value="'.$this->security->get_csrf_hash().'"> 
													   <div class="row col-md-12" >
													   <div class="col-md-12 form-group">
															 <label for="lastname">Particular: <span class="text-danger">*</span></label>
															<select class="form-control select2" name="eye_complaints_id" id="eye_complaints_id"><option value="">Select Particular </option>'.$opvss2.'</select>
														</div>
													</div>
													   <div class="row col-md-12" >
													   <div class="col-md-12 form-group">
	...
	public function updatedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	   $edit_id=trim(htmlentities($this->input->post('id')));
	    		$data=$this->fetch_data();
	    		$getresult=$this->Particular_mapping_model->updatedata($data,$edit_id);
	    		if($getresult)
	    		{
	...
	public function deletedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Particular_mapping_model->deletecheck($var_array);
	    	if($chk_duplication[0]['cnt']==1)
	    	{
	    		$getresult=$this->Particular_mapping_model->deletedata($delete_id);
	    		if($getresult)
	    		{
