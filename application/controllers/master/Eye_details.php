...
class Eye_details extends CI_Controller {
  public function __construct() {
        ...
        $this->load->model('Eye_details_model');
    ...
    public function ajax_call(){
        $param=$_REQUEST;
        $response=$this->Eye_details_model->ajax_call($param);
        echo json_encode($response);
 }
	public function index()
	{
      $data['title']='EMR::Eye_details';
      $data['activecls']='Eye_details';
      $var_array=array($this->session->userdata('office_id'));
	  $data['doctors']=$this->Common_model->getdoctors($var_array);
      $content=$this->load->view('master/eye_details',$data,true);
    ...
    private function fetch_data() 
    {
        ...
        return array(
            'eye_type'=>trim($this->input->post('eye_type')),
    ...
    public function savedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    		...
	    		$getresult=$this->Eye_details_model->savedata($data);
	    		if($getresult)
	    		{
	...
	public function editdata()
	{
		...
		if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$getdata=$this->Eye_details_model->geteditdata($var_array);
	    	if($getdata)
	    	{
	    		$html='';
							                ...
							                <div class="modal-body">
							                    <div class="row">
							                        <div class="col-lg-12 col-md-12" >
							                          <input type="hidden" name="id" id="id" value="'.$data['eye_details_id'].'">
							                           <input type="hidden" name="csrf_test_name" id="csrf_test_name" value="'.$this->security->get_csrf_hash().'"> 
                                                       <div class="row col-md-12">
                                                       <div class="col-md-12 form-group">
                                                           <label for="lastname">Eye Type: </label>
                                                          <select class="form-control" name="eye_type" id="eye_type">
                                                              <option value="1" '.$sel5.'>Left Eye</option>
                                                              <option value="2" '.$sel6.'>Right Eye</option>
                                                          </select>
                                                          </div>
                                                  </div>
							                                 <div class="row col-md-12">
	...
	public function updatedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Eye_details_model->editcheck($var_array);
	    	if($chk_duplication[0]['cnt']==0)
	    	{
	    		$data=$this->fetch_data();
	    		$getresult=$this->Eye_details_model->updatedata($data,$edit_id);
	    		if($getresult)
	    		{
	...
	public function deletedata()
	{
	    ...
	    if($this->form_validation->run() == TRUE)
	    {
	    	...
	    	$chk_duplication=$this->Eye_details_model->deletecheck($var_array);
	    	if($chk_duplication[0]['cnt']==1)
	    	{
	    		$getresult=$this->Eye_details_model->deletedata($delete_id);
	    		if($getresult)
	    		{
