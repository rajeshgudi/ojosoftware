 ...
 * @author Prabhu @15/08/2023

 */

class Dental_examination_model extends CI_Model{
 ...
 public function __construct()
    ...
    public function getSalesSearchStock($product)
  {
    ...
    public function Dental_instruction($var_array)
    {
        $sql = "select name from dental_instruction where  language=$var_array";
        $result_row=$this->db->query($sql);
        $res= $result_row->result_array ();
        return $res;
    }
    public function Get_Showdata_Dia($exid)
    {
        ...
        {
            $sql = "select examination_diagnosis.diagnosis_id,diagnosis.name as dianame,department.name as dename,eye,remarks from examination_diagnosis inner join diagnosis on examination_diagnosis.diagnosis_id=diagnosis.diagnosis_id left join department on diagnosis.department_id=department.department_id where examination_diagnosis.examination_id=0 order by examination_diagnosis_id desc ";
        }
...
// INR new
...

    {
        ...
        $sql = "select examination.doc_action as confirm_flag,examination.patient_registration_id,examination.patient_appointment_id,DATE_FORMAT(examination_date,'%d/%m/%Y') AS opthdate,examination.dialysis_con,examination.examination_id,doctors_registration.name as doctorname,CONCAT(fname , ' ',  lname ,'') AS pateint_name,mrdno,gender,ageyy,agemm,patient_registration.mobileno,examination.doctor_id  from examination inner join patient_registration on patient_registration.patient_registration_id=examination.patient_registration_id left join doctors_registration on doctors_registration.doctors_registration_id=examination.doctor_id where  examination_date=? and doc_action=?  and   examination.office_id= ? and confirm_flag=1 $wh";
...
// echo $this->db->last_query();exit;
 ...
 if($comp_bes)
...

        {
           ...
           $examination_teeth=$data['dental_teeth'];
           $teeths=$examination_teeth['teeth'];
           $teethdates=$examination_teeth['teethdate'];
           $i=0;
           if($teeths)
           {
            foreach ($teeths as $teethsda) 
            {
                 $this->db->insert('dental_teeth',array("examination_id"=>$examination_id,"teeth"=>$teeths[$i],"teethdate"=>date('Y-m-d')));
                 $i++;
            }
           }
           

           $dental_treatment_plans=$data['dental_treatment_plan'];
           $treatment_dates=$dental_treatment_plans['treatment_date'];
           $treatment_plans=$dental_treatment_plans['treatment_plan'];
           $treament_payments=$dental_treatment_plans['treament_payment'];
           $i=0;
           foreach ($treatment_dates as $treatment_datesss) 
           {
                $this->db->insert('dental_treatment',array("examination_id"=>$examination_id,"treatment_date"=>$treatment_dates[$i],"treatment_plan"=>$treatment_plans[$i],"treament_payment"=>$treament_payments[$i]));
                $i++;
           }
          ...
          public function updateexaminationmodel($data,$id)
        {
            ...
            $this->db->where('examination_id', $id);
            $this->db->delete('dental_teeth');
            $this->db->where('examination_id', $id);
            $this->db->delete('dental_treatment');
            $this->db->where('examination_id', $id);
    ...
    public function Getdetailstablefr($exid)
    {
        if($exid)
        {
            $sql = "select * from examination_chargesdetails where  examination_id= $exid";
        }
        else
        {
            $sql = "select * from examination_chargesdetails where  examination_id=0 order by  examination_chargesdetails_id desc";
        }
        $result_row=$this->db->query($sql); 
        $res= $result_row->result_array ();
        $this->logger->save($this->db);
...
public  function print_bill_preview($examinationid,$chkcomplaintsout,$chkopthalmicsout,$chkmedicalout,$chkeyepartout,$addmedicinessout,$investigationchkout,$preliminary_exout,$vsisonreadingsout,$curspecout,$objectchkout,$arkkchkout,$manchkout,$specchkout,$conlchkout,$pmtchkout,$getexamination_treatmentplan_c,$addEyePart_c,$office_id)
{  
                            ...
                            <table width="100%" style="font-size:13px;font-family:Verdana, Arial, Helvetica, sans-serif;margin-top:0%;border-collapse: collapse;border:0.6px solid black;" border="1">
                             <thead>
                                <tr>
                                  ...
                                  <th style="text-align:left;">Compliants</th>
                                  <th>Remarks</th>
                               </tr>
                               </thead>
                               <tbody id="showcomp">';
...

          {
            if($res_teeth)
            {
                     ...
                     $showdata.='<br/><tr>
                           ...
                           <td style="text-align: left;"  class="tabledivideleft" valign="top">Past Dental History:</td>
...

      {
          ...
          $showdata.='<br/><tr>
                       ...
                       <td style="text-align: left;" class="tabledivideright">
                                 ...
                                 <thead><tr><th>SL NO</th><th>Diagnosis</th><th>Department</th></tr></thead><tbody>';
...

            {
...

                 {
                  ...
                  $showdata.='<br/><tr>
                             ...
                             <table  style="width:100%;margin-top:25px;font-size:13px;font-family:Verdana, Arial, Helvetica, sans-serif;margin-top:0%;border-collapse: collapse;border:0.6px solid black;" border="1"><thead><tr><th style="width:5%;">SL NO</th style="width:10%;"><th style="width:20%;">Drug Name</th><th style="width:20%;">Instruction</th><th style="width:20%;">No Of Days</th><th style="width:20%;">Qty</th></tr></thead><tbody>';
...

                   {
                      ...
                      $showdata.='<tr><td>'.$sgl.'</td><td>'.$drugname.'</td><td>'.$datame['instruction'].'</td><td>'.$datame['nodays'].'</td><td>'.$datame['qty'].'</td></tr>';
...
//opth_user_comments
    if($examination_masters->treatment_plan!='')
          {
          $showdata.=    ' <tr>

               <td style="text-align: left;" class="tabledivideleft">Treatment Plan</td>

               <td style="text-align: left;" class="tabledivideright">'.$examination_masters->treatment_plan.'</td> 
        ...
        if($getexamination_treatmentplan_c=='true')
...
<th>Counseling</th>
 ...
 $showdata.='<tr>
  ...
  if($examination_masters->instruction==1)
  {
    $hht='';
    $language_instruc=$this->Dental_instruction(1);
    if($language_instruc)
    {
        $hht.='<h2>Tamil Instruction</h2>';
        foreach($language_instruc as $tamilins)
        {
            $hht.='<p style="font-size:16px;margin-left:12px;">-'.$tamilins['name'].'</p>';
        }
    }
    $language_instruc=$this->Dental_instruction(2);
    if($language_instruc)
    {
        $hht.='<h2>Post Extraction Instruction</h2>';
        foreach($language_instruc as $tamilins)
        {
            $hht.='<p style="font-size:16px;margin-left:12px;">-'.$tamilins['name'].'</p>';
        }
 ...
 $data['conddata']=$showdata;
       ...
       switch($printdata)
          ...
          case 1:$html=$this->load->view("transaction/dental_examination/examinationprint",$data, true); 
...
public function get_teeth_status($doc_examination_id)
{
    ...
    while($i>0) {
      ...
      if($doc_examination_id>0)
      {
          ...
          if($res)
          {
              ...
              if($contval>0)
              {
                  $chl='style="background:yellow;"';
              }
          }
      }
      $htmlloopteeth_rev1.='<td '.$chl.'>
               <label><input type="checkbox" name="teethchk[]"  value="'.$k.'"><span>'.$i.'</span></label>
           </td>';
   ...
   $x=1;
    while($x <= 8) {
      ...
      $htmlloopteeth1.='<td '.$chl.'>
                         <label><input type="checkbox" name="teethchk[]" value="'.$k.'"><span>'.$x.'</span></label>
                     </td>';
     $x++;
     $k++;
   }
   ...
   $x=1;
    while($x <= 8) {
      ...
      if($doc_examination_id>0)
      {
       ...
       $htmlloopteeth2.='<td '.$chl.'>
                          <label><input type="checkbox" name="teethchk[]"  value="'.$k.'"><span>'.$x.'</span></label>
                      </td>';
      $x++;
      $k++;
    }
    $htmll='<tr>'.$htmlloopteeth_rev1.' '.$htmlloopteeth1.'</tr>
   <tr>'.$htmlloopteeth_rev2.' '.$htmlloopteeth2.'</tr>';
   return  $htmll;
}
