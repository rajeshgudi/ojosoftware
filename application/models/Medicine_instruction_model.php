 ...
 *
 * @author Prabhu @11/11/2021
 */
class Medicine_instruction_model extends CI_Model{
 public function __construct()
    ...
    {
        $sql = "select * from medicine_instruction where medicine_instruction_id=? and office_id= ?";
        $result_row=$this->db->query($sql, $var_array); 
        $res= $result_row->result_array ();
        return $res;
    }
    ...
    {
        if($this->db->insert('medicine_instruction',$data))
        {
             return TRUE;
        }
    }
    ...
    {
        $this->db->set($data);
        $this->db->where('medicine_instruction_id', $id);
        if($this->db->update('medicine_instruction'))
        {
            return TRUE;
        }
    }
    ...
    {
        $this->db->where('medicine_instruction_id', $id);
        if($this->db->delete('medicine_instruction'))
        {
            return TRUE;
        }
    }
       ...
       function ajax_call($requestData)
  {
    ...
    $columns = array(
      ...
      0 =>'medicine_instruction_id',
      1 =>'name'
     
    );
 
    $this->db->select('name');//s.photo_no,s.photo_name'
    $this->db->from('medicine_instruction');
    ...
    $sql = "SELECT medicine_instruction_id,name,description,if(status=1,'Active','Deactive') as status";
    $sql.=" FROM medicine_instruction where office_id=$office_id ";
    ...
    if( !empty($requestData['search']['value']) ){   //name
      ...
      for ($i=0; $i < count($row); $i++) {
			$medicine_instruction_id=$row[$i]['medicine_instruction_id'];


	     $edit="<button type='button'  onclick=\"editdata('$medicine_instruction_id');\" class='btn btn-icon btn-warning mr-1 mb-1'><i class='la la-edit'></i></button>";
      	...
      	$delete='<button onclick="deletedata('.$medicine_instruction_id.')" type="button" class="btn btn-icon btn-danger mr-1 mb-1"><i class="la la-trash"></i></button>';
