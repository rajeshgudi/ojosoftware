 ...
 * @author Prabhu @25/07/2023

 */

class Insurance_billing_model extends CI_Model{
 ...
 public function __construct()

	{
       ...
       $last_invoice_number=$this->db->select('max(insurance_bill_number) as last_invoice_number')
                         ->from('insurance_bill_number')
                         ->where(array('office_id'=>$this->session->office_id))
        ...
        $this->db->insert('insurance_billing_master',$billing_masters);
        $billing_master_id=$this->db->insert_id();
        $office_id=$this->session->userdata('office_id');
        $login_id=$this->session->userdata('login_id');
        $bill_numbers=$data['bill_number'];
        $bill_numbers['insurance_bill_number']=$bill_number;
        $bill_numbers['master_id']=$billing_master_id;
        $this->db->insert('insurance_bill_number',$bill_numbers);
        ...
        $i=0;
           ...
           {
                    $this->db->insert('insurance_billing_detail',array(
...

  {
    ...
    $this->db->from('insurance_billing_master');
    ...
    $sql = "SELECT insurance_billing_master_id,insurance_billing_master.invoice_number,CONCAT(fname , '',  lname ,'') AS pateint_name,billing_date,bill_amount,advanced_amount,(bill_amount-(adjusted_amount_paid+advanced_amount)) as pending,grand_total,modeofpay.name as mode";

    $sql.=" FROM insurance_billing_master left join patient_registration on patient_registration.patient_registration_id=insurance_billing_master.patient_registration_id left join modeofpay on insurance_billing_master.modeofpay_id=modeofpay.modeofpay_id  where insurance_billing_master.office_id=$office_id ";
    ...
    if( !empty($requestData['search']['value']) ){   //name
      ...
      $sql.="  and (  ";
        ...
        $sql.="  OR insurance_billing_master.patient_registration_id in (select patient_registration_id from patient_registration  where (fname  LIKE '".$requestData['search']['value']."%' or lname  LIKE '".$requestData['search']['value']."%' ) ) ";
      ...
      for ($i=0; $i < count($row); $i++) {
            ...
            $billing_master_id=$row[$i]['insurance_billing_master_id'];
 ...
 $print='<button onclick="printdata('.$billing_master_id.')" type="button" class="btn btn-icon btn-info mr-1 mb-1"><i class="la la-print"></i></button>';
  ...
  public function editcheckbillingentry($var_array)
...

    {
        ...
        $sql = "select * from insurance_billing_master where  insurance_billing_master_id= ? and  office_id= ?";
...

    {
        ...
        $sql = "select * from insurance_billing_detail where  billing_master_id= ?";
     ...
     public function updatedata($data,$id)
        {
            ...
            $this->db->delete('insurance_billing_detail');
            $billing_masters=$data['billing_master'];
            $this->db->set($billing_masters);
            $this->db->where('insurance_billing_master_id', $id);
            $this->db->update('insurance_billing_master');
         ...
         public function deletedata($id) 
        {
            ...
            $this->db->delete('insurance_billing_detail');
            $this->db->where('insurance_billing_master_id',"$id");
            $this->db->delete('insurance_billing_master');
           if ($this->db->trans_status() === FALSE)
            {
                    $this->db->trans_rollback();
                    return FALSE;
  ...
  { error_reporting(0);
    ...
    $billing_masters=$this->db->get_where('insurance_billing_master',"insurance_billing_master_id=$billing_id")->row();
   ...
   $data['ins_name']=$this->db->get_where('insurance_company',"insurance_company_id=$billing_masters->insurance_company_id")->row()->name;
...
$data['dpaymentm']=$paymentmo; 
   ...
   if($billing_masters->updated_date)
    ...
    $data['acc_holdername']=$billing_masters->acc_holdername; 
    $data['bank_name']=$billing_masters->bank_name; 
    $data['ifs_code']=$billing_masters->ifs_code; 
    $data['claimno']=$billing_masters->claimno; 
    $data['acc_no']=$billing_masters->acc_no; 
   

    $data['billid']=$billing_id;
...
$host_tvm = explode('.',$_SERVER['HTTP_HOST'])[0];
   if($host_tvm=='queens')
   {
          ...
          $html=$this->load->view("transaction/insurance_billing/".$pri,$data, true); 
                   $print_config=[
                                    'format' => [100,200],
                                    'margin_left' => 10,
                                    'margin_right' => 10,
   ...
   else
   {
         ...
         switch($patient_details->print)
            ...
            case 1:$html=$this->load->view("transaction/insurance_billing/".$pri,$data, true); 
            ...
            case 2:$html=$this->load->view("transaction/insurance_billing/".$pri,$data, true); 
           ...
           case 3:$html=$this->load->view("transaction/insurance_billing/".$pri,$data, true); 
              ...
              case 4:$html=$this->load->view("transaction/insurance_billing/".$pri,$data, true); 
