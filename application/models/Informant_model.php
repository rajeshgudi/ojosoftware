...
class Informant_model extends CI_Model{
 public function __construct()
	...
	{
		$sql = "select count(*) as cnt from informant where  name = ? and office_id= ?";
    ...
    {
        if($this->db->insert('informant',$data))
        {
             return TRUE;
        }
    }
    ...
    {
        $this->db->set($data);
        $this->db->where('informant_id', $id);
        if($this->db->update('informant'))
        {
            return TRUE;
        }
    }
       ...
       function ajax_call($requestData)
  {
    ...
    $sql = "SELECT informant_id,name,description,if(status=1,'Active','Deactive') as status";
    $sql.=" FROM informant where office_id=$office_id ";
    ...
    if( !empty($requestData['search']['value']) ){   //name
      ...
      for ($i=0; $i < count($row); $i++) {
			$informant_id=$row[$i]['informant_id'];


	     $edit="<button type='button'  onclick=\"editdata('$informant_id');\" class='btn btn-icon btn-warning mr-1 mb-1'><i class='la la-edit'></i></button>";

      	// $edit='<button  onclick="editinformant_id('.$informant_id_id.',1,$(this),'.$code.','.$name.','.$description.','.$status.')" type="button" class="btn btn-icon btn-warning mr-1 mb-1"><i class="la la-edit"></i></button>';

      	$delete='<button onclick="deletedata('.$informant_id.')" type="button" class="btn btn-icon btn-danger mr-1 mb-1"><i class="la la-trash"></i></button>';

        // if(($this->auth->lock_up('item_master',"category_id='$informant_id_id'")) || ($this->auth->lock_up('supplier',"category_id='$informant_id_id'")) ||in_array($informant_id_id,[1,2,3]))
        //   {
        //       $delete='<button type="button" class="btn btn-icon btn-primary mr-1 mb-1"><i class="la la-lock"></i></button>';
        //   }
        //   if(in_array($informant_id_id,[1,2,3]))
        //   {
