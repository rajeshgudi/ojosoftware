 ...
 */
class Coating_model extends CI_Model{
 public function __construct()
    ...
    {
        $sql = "select * from coating where coating_id=? and office_id= ?";
        $result_row=$this->db->query($sql, $var_array); 
        $res= $result_row->result_array ();
        return $res;
    }
    ...
    {
        if($this->db->insert('coating',$data))
        {
             return TRUE;
        }
    }
    ...
    {
        $this->db->set($data);
        $this->db->where('coating_id', $id);
        if($this->db->update('coating'))
        {
            return TRUE;
        }
    }
       ...
       function ajax_call($requestData)
  {
    ...
    $sql = "SELECT coating_id,name,description,if(status=1,'Active','Deactive') as status";
    $sql.=" FROM coating where office_id=$office_id ";
    ...
    if( !empty($requestData['search']['value']) ){   //name
      ...
      for ($i=0; $i < count($row); $i++) {
			$coating_id=$row[$i]['coating_id'];


	     $edit="<button type='button'  onclick=\"editdata('$coating_id');\" class='btn btn-icon btn-warning mr-1 mb-1'><i class='la la-edit'></i></button>";

      	// $edit='<button  onclick="editcoating_id('.$coating_id_id.',1,$(this),'.$code.','.$name.','.$description.','.$status.')" type="button" class="btn btn-icon btn-warning mr-1 mb-1"><i class="la la-edit"></i></button>';
        ...
        // if(($this->auth->lock_up('item_master',"category_id='$coating_id_id'")) || ($this->auth->lock_up('supplier',"category_id='$coating_id_id'")) ||in_array($coating_id_id,[1,2,3]))
        //   {
        //       $delete='<button type="button" class="btn btn-icon btn-primary mr-1 mb-1"><i class="la la-lock"></i></button>';
        //   }
        //   if(in_array($coating_id_id,[1,2,3]))
        //   {
