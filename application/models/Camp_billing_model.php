...
class Camp_billing_model extends CI_Model{
 ...
 public function __construct()

	{
       ...
       $last_invoice_number=$this->db->select('max(bill_number) as last_invoice_number')
         ...
         $invoice_number='CAMP-'.str_pad($invoice_number, strlen($bill_setting->padding),$bill_setting->padding,STR_PAD_LEFT);
           ...
           $this->db->insert('camp_billing_master',$billing_masters);
           ...
           $this->db->insert('camp_bill_number',$bill_numbers);
...

           {
                    ...
                    $this->db->insert('camp_billing_detail',array(
...

  {
    ...
    $this->db->from('camp_billing_master');
    ...
    $sql = "SELECT billing_master_id,camp_billing_master.invoice_number,namecamp,billing_date,bill_amount,advanced_amount,(bill_amount-(adjusted_amount_paid+advanced_amount)) as pending,grand_total,modeofpay.name as mode";

    $sql.=" FROM camp_billing_master left join camp_deh on camp_deh.camp_deh_id=camp_billing_master.patient_registration_id left join modeofpay on camp_billing_master.modeofpay_id=modeofpay.modeofpay_id  where camp_billing_master.office_id=$office_id ";
    ...
    if( !empty($requestData['search']['value']) ){   //name
      ...
      $sql.="  and (  ";
        ...
        $sql.="  OR camp_billing_master.patient_registration_id in (select patient_registration_id from patient_registration  where (fname  LIKE '".$requestData['search']['value']."%' or lname  LIKE '".$requestData['search']['value']."%' ) ) ";
...
public function getpateintmrdnosa($var_array)

    {
        ...
        $sql = "select * from camp_deh  order by camp_deh_id desc";
  ...
  public function editcheckbillingentry($var_array)
...

    {
        ...
        $sql = "select * from camp_billing_master where  billing_master_id= ? and  office_id= ?";
...

    {
        ...
        $sql = "select * from camp_billing_detail where  billing_master_id= ?";
...

    {
     ...
     public function updatedata($data,$id)

        {
            ...
            $this->db->delete('camp_billing_detail');
            ...
            $this->db->update('camp_billing_master');
...

        {
            ...
            $this->db->delete('camp_billing_master');
  ...
  { 
     ...
     $billing_masters=$this->db->get_where('camp_billing_master',"billing_master_id=$billing_id")->row();
     ...
     $patient_details=$this->db->get_where('camp_deh',"camp_deh_id=$billing_masters->patient_registration_id")->row();
     ...
     $charge_id=$this->db->get_where('camp_billing_detail',"billing_master_id=$billing_id")->row()->charge_id;
    ...
    $data['fname']=$patient_details->namecamp; 
    $data['lname']=''; 
    $data['mrdno']=$patient_details->mrdno; 
    $data['address']=''; 
    $data['mobileno']=$patient_details->mobileno; 
    $data['date_of_admission']=$patient_details->camp_date;
    ...
    $data['particulars_details']=$this->db->query("select * from camp_billing_detail left join package_master on package_master.package_master_id=camp_billing_detail.package_id where billing_master_id=$billing_id")->result();  
    $html=$this->load->view("transaction/camp_billing/".$pri,$data, true); 
